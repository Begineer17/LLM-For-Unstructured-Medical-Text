...
- Derived linking index được tạo dưới `data/ontology/derived/`, kiểm tra checksum/count của raw snapshot và giữ alias curated có provenance; raw ontology không bị sửa.
...
### Tạo derived ontology index và audit output

```bash
python scripts/build_derived_ontology.py
python scripts/evaluate_outputs.py --input input_2x --output output_2x
```

Derived index chỉ là artifact dẫn xuất; `data/ontology/icd/concepts.jsonl` và
`data/ontology/rxnorm/concepts.jsonl` vẫn là snapshot nguồn. Audit đọc output đã
có, không gọi LLM và không mở endpoint.
...
`clinical_nlp_pipeline.py` chứa schema, detector, assertion resolver, local-LLM adapter, resolver và serializer. `ontology_linking.py` chứa derived index, alias validation và scored linker. `scripts/` cung cấp CLI inference, validator, ontology builder, output evaluator và synthetic generator. Thiết kế không hard-code tên file, bệnh, thuốc hay sample cụ thể; mọi đường dẫn truyền qua CLI.
